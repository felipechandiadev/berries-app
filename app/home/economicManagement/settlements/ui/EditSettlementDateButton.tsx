...
import { updateSettlementDate } from '@/app/actions/settlements';
import { useAlert } from '@/app/state/hooks/useAlert';

interface EditSettlementDateButtonProps {
  settlementId: string;
  currentDate: string;
  producerName?: string;
  onSuccess?: () => void;
...
const EditSettlementDateButton = ({
  settlementId,
  currentDate,
  producerName,
  onSuccess,
}: EditSettlementDateButtonProps) => {
  ...
  const handleSubmit = useCallback(
    async (values: Record<string, unknown>) => {
      ...
      const result = await updateSettlementDate({
        settlementId,
        newDate: newIsoDate,
      ...
      success('Fecha de la liquidación actualizada correctamente.');
      handleCloseDialog();
      router.refresh();
      onSuccess?.();
    },
    [settlementId, currentDate, currentUserId, success, handleCloseDialog, router, onSuccess]
  );
  ...
  return (
    <>
      <IconButton
        ...
        variant="text"
        size="sm"
        data-test-id={`edit-settlement-date-button-${settlementId}`}
      />
      ...
      <Dialog
        ...
        maxWidth="sm"
        data-test-id="edit-settlement-date-dialog"
      >
        <div className="space-y-4">
          {producerName && (
            <p className="text-sm text-gray-600">
              Liquidación para: <span className="font-medium">{producerName}</span>
            </p>
          )}
...
export default EditSettlementDateButton;
