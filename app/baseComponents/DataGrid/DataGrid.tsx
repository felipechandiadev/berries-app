...
export interface DataGridProps {
  ...
  expandedRowIds?: Set<string | number>; // IDs de filas expandidas actuales
  onToggleExpand?: (rowId: string | number) => void; // Callback para alternar expansión
  expandedRowId?: string | number | null; // For backward compatibility
}
...
const DataGrid: React.FC<DataGridProps> = ({
  ...
  const [expandedRowIdsInternal, setExpandedRowIdsInternal] = useState<Set<string | number>>(new Set(defaultExpandedRowIds));
  ...
  // Toggle expandir/colapsar una fila (internal management)
  const toggleRowExpandedInternal = (rowId: string | number) => {
    setExpandedRowIdsInternal(prev => {
  ...
  // Use external or internal expandable state
  const currentExpandedRowIds = expandedRowIds || expandedRowIdsInternal;
  const currentToggleExpand = onToggleExpand || toggleRowExpandedInternal;
  ...
  return (
    <div className={containerClasses} style={{ height: typeof height === 'number' ? `${height}px` : height }} data-test-id={dataTestId || "data-grid-root"}>
      ...
      <div className={`${DataGridStyles.scrollContainer} relative`}>
        ...
        >
          {/* Expand button header column - empty space for alignment */}
          {expandable && (
            <div className="w-10 min-w-[40px] px-3 py-2 border-b border-gray-200"></div>
          )}
        ...
        <Body 
          ...
          onToggleExpand={currentToggleExpand}
